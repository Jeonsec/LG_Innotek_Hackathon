...
class AUCM_MultiLabel(torch.nn.Module):
    ...
    def __init__(self, margin=1.0, imratio=[0.1], num_classes=10, device=None):
        ...
        assert (
            len(imratio) == num_classes
        ), "Length of imratio needs to be same as num_classes!"
        self.a = torch.zeros(
            num_classes, dtype=torch.float32, device="cuda", requires_grad=True
        ).to(self.device)
        self.b = torch.zeros(
            num_classes, dtype=torch.float32, device="cuda", requires_grad=True
        ).to(self.device)
    ...
    def forward(self, y_pred, y_true):
        ...
        for idx in range(self.num_classes):
            ...
            loss = (
                (1 - self.p[idx])
                * torch.mean((y_pred_i - self.a[idx]) ** 2 * (1 == y_true_i).float())
                + self.p[idx]
                * torch.mean((y_pred_i - self.b[idx]) ** 2 * (0 == y_true_i).float())
                + 2
                * self.alpha[idx]
                * (
                    ...
                    + torch.mean(
                        (
                            self.p[idx] * y_pred_i * (0 == y_true_i).float()
...
class APLoss_SH(torch.nn.Module):
    def __init__(
        self, data_len=None, margin=1.0, beta=0.99, batch_size=128, device=None
    ):
        """
    ...
    def forward(self, y_pred, y_true, index_s):
        ...
        neg_loss = (
            torch.max(self.margin - (f_ps - mat_data), torch.zeros_like(mat_data)) ** 2
            * neg_mask
        )
        pos_loss = (
            torch.max(self.margin - (f_ps - mat_data), torch.zeros_like(mat_data)) ** 2
        ...
        if f_ps.size(0) == 1:
            self.u_pos[index_s] = (1 - self.beta) * self.u_pos[index_s] + self.beta * (
                pos_loss.mean()
            )
            self.u_all[index_s] = (1 - self.beta) * self.u_all[index_s] + self.beta * (
                loss.mean()
            )
        else:
            self.u_all[index_s] = (1 - self.beta) * self.u_all[index_s] + self.beta * (
                loss.mean(1, keepdim=True)
            )
            self.u_pos[index_s] = (1 - self.beta) * self.u_pos[index_s] + self.beta * (
                pos_loss.mean(1, keepdim=True)
            )

        p = (self.u_pos[index_s] - (self.u_all[index_s]) * pos_mask) / (
            self.u_all[index_s] ** 2
        )
