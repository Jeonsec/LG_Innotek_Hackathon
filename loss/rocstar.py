...
class ROCstarLoss(nn.Module):
    def __init__(self):
        ...
        self.pred = []
        self.true = []
        self.epoch_gamma = [0.20, 0.20, 0.20, 0.20]
        self.epoch = 0
        self.classes_cls = 4
    ...
    def forward(self, x: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        if self.epoch > -1:
            ...
            return self.__roc_star_loss(target, x, self.epoch_gamma, self.true, self.pred)

    @classmethod
    def update_epoch(self):
        for i in range(self.classes_cls):
            self.epoch_gamma[i] = self.__epoch_update_gamma(self.true[:, i], self.pred[:, i], self.epoch)

    @classmethod
    def __epoch_update_gamma(y_true, y_pred, epoch=-1, delta=1):
        """
    ...
    @classmethod
    def __roc_star_loss(_y_true, y_pred, gamma, _epoch_true, epoch_pred):
        """
